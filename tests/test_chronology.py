...
from hscoach.replay.timeline import _packet_protocol_orders, extract_timeline
...
def test_packet_tree_alignment_rejects_a_single_xml_mismatch_without_shifting() -> None:
    context = parse_replay_data(MINIMAL_REPLAY.read_bytes())
    first_tag_change = next(context.game_xml.iter("TagChange"))
    first_tag_change.tag = "UnsupportedTagChange"
...
def test_choice_zero_sentinel_does_not_become_earliest_without_common_order() -> None:
    ...
    turn = SimpleNamespace(actions=actions, entity_deltas=[])
    _attach_choice_actions(turn, [choice], {})
    ...
    assert [action.sequence for action in turn.actions] == [1, 2, 3]
